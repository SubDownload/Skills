...
5. **Saving transcript + summary in one call.** After you've generated an AI summary for a video you just transcribed, call `save_to_library` with `kind='both'` — it flips `has_asr` AND uploads the summary text in one round-trip (still one quota unit). Use `kind='asr'` for transcript-only, `kind='summary'` for summary-only. Pass `format='markdown'` when your summary uses `**bold**`, bullets, headings, or code fences; pass `format='text'` for plain prose. Default is `markdown`. The web UI at subdownload.com reads this to pick the right renderer, so getting it right means the user sees a formatted summary instead of literal `**` marks.
